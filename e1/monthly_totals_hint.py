...
def pivot_months_pandas(data):
    ...
    """
    # ...
    # Create shortened dates
    dates = data['date']
    dates2 = dates.apply(date_to_month)
    dates2 = pd.DataFrame(data=dates2)
    dates2.columns = ['month']

    # Concat & pivot
    data = pd.concat([data['name'], dates2, data['precipitation']], axis=1)

    temp_data = data
    data = data.groupby(['name', 'month']).sum().reset_index()

    data2 = temp_data.groupby(['name', 'month']).count().reset_index()

    data = data.pivot(index='name', columns='month', values='precipitation')
    data.columns.name = ''
